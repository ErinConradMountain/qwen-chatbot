...
try:
    ...
    pass  # OS env will still be used by providers

_requests: Optional[Any]
try:
    import requests as _requests  # Only used for Ollama; requests is ubiquitous
except Exception:
    _requests = None

requests = cast(Optional[Any], _requests)
...
def openai_infer(model: Optional[str], messages: List[Message]) -> str:
    ...
    serializable_messages = cast(Sequence[Any], chat_messages)

    try:
        response = client.chat.completions.create(
            model=model, messages=serializable_messages
        )
